...
filter_list = ['transaction_date', 'sales_outlet_id', 'quantity', 'unit_price']
...
def sales_monthly_bases(sales_outlet_id, year, month):
    filtered_df = df[filter_list]

    monthly_sales_df = filtered_df[
        (filtered_df["sales_outlet_id"] == sales_outlet_id) &
        (filtered_df["transaction_date"].dt.year == year) &
        (filtered_df["transaction_date"].dt.month == month)
        ].copy()
    monthly_sales_df.loc[:, "total_sales"] = monthly_sales_df["quantity"] * monthly_sales_df["unit_price"]
    print(monthly_sales_df["total_sales"].sum())
# Example usage
# day_date = "2019-04-01"
# print(sales_daily_base(3, day_date))
sales_monthly_bases(5, 2019, 4)